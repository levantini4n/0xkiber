...
\begin{center}
    ...
    \captionof{figure}{A closed TCP port responds to a SYN packet with RST/ACK to indicate that it is not open.}
    \label{fig:sT-scan}
    \end{center}
    \begin{center}
        \includegraphics[width=0.7\textwidth]{stealth-scan.png}
        \captionof{figure}{SYN scan does not need to complete the TCP 3-way handshake; instead, it tears down the connection once it receives a response from the server.}
        \label{fig:sS-scan}
        \end{center}

            \begin{center}
                \includegraphics[width=0.7\textwidth]{sU-success.png}
                \captionof{figure}{the UDP ports that don’t generate any response are the ones that Nmap will state as open.}
